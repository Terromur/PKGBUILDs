...
Был изменён wine-pure-clang-git:
- Все теже изменения, что и для wine-pure-git
...
- Добавлен патч для управления alsa периодами через переменную, например ```export STAGING_AUDIO_PERIOD=5000```(5 мс)
- Внимание! Для gcc или обычного PKGBUILD рекомендуется установить мой mingw-w64-gcc из директории или готовый мой пакет для исправления Хогвартс Легаси и подобных игр
